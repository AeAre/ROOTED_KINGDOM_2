...
func _ready():
	...
	if stage_count_label:
		stage_count_label.text = "Stage: " + str(Global._CURRENTLY_PLAYING_CHAPTER) + " - " + str(Global._current_playing_on_stage)
...
func setup_tower_enemies():
	...
	var selected_stage_data: Array[EnemyData] = []
	
	# --- 1. CALCULATE GROWTH ---
	var growth_percent = 0.05 
	var growth_multiplier = 1.0 + (StoryMode.multiplier[StoryMode.stage_label()] * growth_percent)
	...
	for i in range(selected_stage_data.size()):
		if i < enemy_nodes.size():
			var enemy_resource = selected_stage_data[i]
