...
@export var availablity: bool

@onready var availablity_label = $Interface/Availability
...
@onready var stage_button =  $"Interface/Stage Button"

# Called when the node enters the scene tree for the first time.
func _ready() -> void:
	$"Interface/Stage Button".text = str(Global._CURRENTLY_PLAYING_CHAPTER) + " - " + str(Stage_number)
	...
	if availablity:
		availablity_label.visible = false
		stage_button.disabled = false
	else:
		availablity_label.visible = true
